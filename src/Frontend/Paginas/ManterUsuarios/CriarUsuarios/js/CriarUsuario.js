document.getElementById("formCriarUsuario").addEventListener("submit", function(event) {
    ...
    if (isValidInput(matricula, nome, senha)) {
        ...
        displayErrorMessage("Verifique os dados inseridos: Matrícula deve ter 4 dígitos, Nível de Acesso 1 dígito e Senha com pelo menos 8 caracteres.");
    }
});
...
function sendFormData(matricula, nome, senha) {
    ...
    xhr.open("POST", "/Classement_backend/ManterUsuarios/criar_usuario.php", true);
...
function handleResponse(response) {
    if (response.status === 'success') {
        ...
        displayErrorMessage("Erro ao criar usuário.");
    }
}
